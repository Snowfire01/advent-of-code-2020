...
function partOne() {
    ...
    for (line of LINES) {
        ...
        for (kv of kvStrings) {
            let groups = /(\w+):([#\w\d]+)/.exec(kv).slice(1)
            object[groups[0]] = groups[1]
        }
...
function partTwo() {
    ...
    for (line of LINES) {
        ...
        if (match) {
            ...
            let byr = +match[2]
            let eyr = +match[3]
            let hgtValue = +match[4]
            let hgtUnit = match[5]

            if (iyr.between(2010, 2020) && byr.between(1920, 2002) && eyr.between(2020, 2030)
                && hgtValue.between(hgtUnit === "cm" ? 150 : 59, hgtUnit === "cm" ? 193 : 76))
                valid++
        }
    }
